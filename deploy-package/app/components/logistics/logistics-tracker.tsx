...
export function LogisticsTracker() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      setSupabaseConfigured(!!getClient())
      // 如果配置成功，自动开启同步
      if (getClient()) {
        setSyncEnabled(true)
      }
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="text-sm">
              <span className="font-medium">云同步状态：</span>
              {supabaseConfigured ? (
                <span className="text-green-600">✅ 已连接 Supabase</span>
              ) : (
            ...
            {supabaseConfigured && (
              <>
                <button
                  ...
                  title={syncEnabled ? '点击暂停同步' : '点击开启同步'}
                >
