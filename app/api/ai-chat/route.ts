...
async function callCherryStudioAgent(message: string, config: AIConfig) {
  try {
    ...
    // Cherry/AiHubMix 目前常见可用模型（2025）：gpt-4o、gpt-4.1、gpt-4
    ...
    const apiKey = config.apiKey || process.env.AIHUBMIX_API_KEY || ''

    if (!apiKey) {
      return NextResponse.json(
        { error: '未配置AIHUBMIX_API_KEY，请在前端设置里填写，或在服务器端添加环境变量。' },
        { status: 400 }
      )
    }
